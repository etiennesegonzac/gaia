...
suite('system/EdgeSwipeDetector >', function() {
  ...
  suite('Touch handling > ', function() {
    ...
    function swipe(clock, panel, fromX, toX, fromY, toY, tick, noEnd) {
      var tick = tick || 25;
    ...
    setup(function() {
      ...
      this.sinon.stub(MockStackManager, 'getCurrent').returns({
        origin: 'app://app.gaiamobile.org'
      });
      this.sinon.stub(MockWindowManager, 'getAppFrame').returns({
        firstChild: iframe
      });
    ...
    suite('Snaping >', function() {
      suite('when the progress was < 33%', function() {
        ...
        suite('but there is inertia', function() {
          test('it should snap the sheets back', function() {
            ...
            swipe(this.sinon.clock, panel, 3, (width / 4), 240, 250, 4);
            assert.isTrue(snapSpy.calledOnce);
          });
          ...
          test('it should pass the speed to snapBack', function() {
            ...
            assert.isTrue(givenSpeed > 0.00075);
            assert.isTrue(givenSpeed < 0.00080);
          });
      ...
      suite('when the progress was > 33% ltr', function() {
        ...
        test('it should pass the speed to snapBack', function() {
          ...
          assert.isTrue(givenSpeed > 0.00010);
          assert.isTrue(givenSpeed < 0.00015);
        });
